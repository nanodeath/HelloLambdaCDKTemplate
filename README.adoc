...
* *Java 21*, the latest Java LTS which is https://aws.amazon.com/about-aws/whats-new/2023/11/aws-lambda-support-java-21/[now supported] on AWS Lambda.
...
* `java -version` -- running this should print out a version string for Java 21. Otherwise, you'll need to find and install it.
* `node --version` -- running this should print out a v20 (or later) version string, like `v20.14.0`. If not, consider using something like https://github.com/nvm-sh/nvm[nvm] to install and use Node. You're also assumed to have `npm` on your path.
...
After stack teardown is complete, display `Deleted` stacks in the CloudFormation console and select the stack you just deleted. In the Resources tab, if there are any resources that aren't `DELETE_COMPLETE`, you may need to delete those resources manually -- especially things like CloudFormation LogGroups for the Lambdas.
